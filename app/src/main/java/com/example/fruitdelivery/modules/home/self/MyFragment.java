...
import com.example.fruitdelivery.util.LogUtil;
...
public class MyFragment extends BaseFragment<MyPresenter> implements ISelfView {
     ...
     * 判断登录状态
     */
    private boolean boolLoginState = false;

    /**
     * M层获取到的数据
     */
    JsonRootBean mJsonRootBean;
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        myHead.setOnClickListener(new NoDoubleClickListener() {
            ...
            protected void onNoDoubleClick(View v) {
                //先判断是否登录状态，假设true为登录，false为未登录
                if (boolLoginState == false){
                    startActivity(new Intent(getContext(), LoginActivity.class));
                }else if(boolLoginState == true){
                    Toast.makeText(getContext(),"跳转至修改个人信息页",Toast.LENGTH_SHORT).show();
                }
            }
        });
...
//    得到数据
    ...
    public void setMyData(JsonRootBean jsonRootBean) {
        this.mJsonRootBean = jsonRootBean;
    }
